...
from thumbzilla_api.api import create_parser, run_main
...
class TestCLI(unittest.IsolatedAsyncioTestCase):
    ...
    def test_parser_custom_args(self):
        ...
        args = parser.parse_args([
            "--download", "https://thumbzilla.com/video/123/test",
            "--output", "/tmp/out",
            "--quality", "worst",
            "--no-title", "True"
        ])
        self.assertEqual(args.download, "https://thumbzilla.com/video/123/test")
        self.assertEqual(args.output, "/tmp/out")
    ...
    async def test_run_main_download(self):
        ...
        with patch("thumbzilla_api.api.Client", return_value=mock_client):
            await run_main(["--download", "https://thumbzilla.com/video/123/test", "--output", "/tmp/out"])

        mock_client.get_video.assert_awaited_once_with("https://thumbzilla.com/video/123/test", load_html=True)
    ...
    async def test_run_main_file(self):
        ...
        with tempfile.NamedTemporaryFile("w+", delete=False) as f:
            f.write("https://thumbzilla.com/1\nhttps://thumbzilla.com/2\n")
            f.flush()
            temp_name = f.name
    ...
    async def test_real_download(self):
        ...
        url = "https://www.thumbzilla.com/watch/225766831/"
